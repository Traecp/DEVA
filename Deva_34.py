...
class PopUpBox(object):
	def __init__(self):
		...
		self.XRDML_run.connect("clicked", self.Validate_Param)
...
class MyMainWindow(gtk.Window):
	...
	def __init__(self):
		...
		#**************************** Geometry setup ******************************************
		self.geometry_setup_tbl = gtk.Table(4,4,True)
		...
		self.hkl_btn.connect("toggled", self.plot_update)
...
######################################## Definitions ########################################################################
	...
	def init_image(self):
		...
		if self.tth_chi_space_btn.get_active():
			...
			self.MAIN_XLABEL.set_text(r"$Qy\ (AA)")
			self.MAIN_YLABEL.set_text("Chi (deg.)")
	...
	def manual_calibration(self,widget):
		...
		if self.geometry_manual.get_active():
			...
			if in_plane != "" and out_of_plane != "":
				...
				self.experiment = xrayutilities.HXRD(self.substrate.Q(1,1,0),self.substrate.Q(0,0,1), en=energy, qconv=qconv)
	...
	def HKL_Plot(self,w):
		if self.q_space_btn.get_active():
			self.MAIN_XLABEL.set_text("K (r.l.u.)")
			self.MAIN_YLABEL.set_text("L (r.l.u.)")
			self.plot_hkl()
		else:
			self.plot_update(w)
	...
	def Reciprocal_space_plot(self):
		#if self.q_space_btn.get_active():
		self.tth_chi_space_btn.set_active(False)
		if self.calibrated == False:
			self.popup_info('warning','Please calibrate the detector before checking this!')
