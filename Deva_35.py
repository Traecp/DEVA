...
class MyMainWindow(gtk.Window):
	...
	def __init__(self):
		...
		self.geometry_manual    = gtk.ToggleButton("VALIDATE the above parameters setup")
		...
		self.hkl_space_btn = gtk.CheckButton("HKL space")
		self.hkl_space_btn.connect("toggled", self.plot_update)
		...
		self.option_table.attach(self.hkl_space_btn, 1,2,2,3)
...
######################################## Definitions ########################################################################
	...
	def init_image(self):
		...
		if self.tth_chi_space_btn.get_active():
			self.MAIN_XLABEL.set_text(r"$2Theta (deg.)$")
			self.MAIN_YLABEL.set_text(r"$Chi (deg.)$")
		
		elif self.q_space_btn.get_active():
			self.MAIN_XLABEL.set_text(r"$Qy\ (\AA^{-1})$")
			self.MAIN_YLABEL.set_text(r"$Qz\ (\AA^{-1})$")
		elif self.hkl_space_btn.get_active():
	...
	def on_changed_edf(self,widget,row,col):
		...
		if self.detector_type != "D1":
			...
			if manip == "fourc":
				self.del_pos.set_text("%s: %.2f"%("tth",self.delta))
				self.eta_pos.set_text("%s: %.2f"%("th",self.eta))
				self.phi_pos.set_text("%s: %.2f"%("phi",self.phi))
				self.kphi_pos.set_text("%s: %.2f"%("kphi",self.kphi))
				self.chi_pos.set_text("%s: %.2f"%("chi",self.chi))
				self.nu_pos.set_text("%s: %.2f"%("nu",self.nu))
	...
	def reset_image(self,widget):
		...
		if self.q_space_btn.get_active() or self.hkl_space_btn.get_active():
	...
	def Angular_space_plot(self):
		#if self.tth_chi_space_btn.get_active():
		self.hkl_space_btn.set_active(False)
		self.q_space_btn.set_active(False)
	...
	def Reciprocal_space_plot(self,space="Q"):
		#space="Q", or "HKL"
		self.tth_chi_space_btn.set_active(False)
		if self.calibrated == False:
			self.popup_info('warning','Please calibrate the detector before checking this!')
			...
			if flag:
				if self.UB_MATRIX_LOAD:
					...
					self.QGridder = xrayutilities.Gridder2D(1000, 500)
					if space=="Q":
						self.hkl_space_btn.set_active(False)
	...
	def plot_data(self):
		...
		if self.tth_chi_space_btn.get_active():
			...
			self.Reciprocal_space_plot(space="HKL")
		else:
	...
	def plot_profiles(self, x, y, cross_line=True):
		...
		if cross_line:
			...
			if self.tth_chi_space_btn.get_active():
				...
				X_label = "Qy (1/Angstrom)"
				Y_label = "Qz (1/Angstrom)"
			...
			elif self.q_space_btn.get_active() or self.hkl_space_btn.get_active():
			...
			if self.tth_chi_space_btn.get_active():
				...
				if self.q_space_btn.get_active():
					X_label = "Qy (1/Angstrom)"
					Y_label = "Qz (1/Angstrom)"
					self.chi_title.set_text("Qz")
					self.tth_title.set_text("Qy")
